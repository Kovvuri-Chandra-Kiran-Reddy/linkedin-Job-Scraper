...
class JobSearch(Scraper):
    ...
    def search(self, search_term: str) -> List[Job]:
        first_url = os.path.join(self.base_url, "search") + f"?keywords={urllib.parse.quote(search_term)}&refresh=true"
        urls = [first_url, first_url+'&start=25']
        job_results = []
        count = 1
        for url in urls:
            print(f"Scraping {search_term} job pages {count} of 2....\n")
            self.driver.get(url)
